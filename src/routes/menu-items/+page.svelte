<script>
	...
	async function saveItem() {
		if (loading) return;
		loading = true;
		//saveItem funciona para crear o actualizar un ítem de menú
			...
			<form on:submit|preventDefault={saveItem}>
				<div class="grid grid-cols-1 gap-4">
					<div>
						...
						<select
							id="category_id"
							bind:value={currentItem.category_id}
							required
					...
					<div>
						...
						<input
							type="number"
							id="order_index"
							bind:value={currentItem.order_index}
							min="0"
							class="w-full rounded border px-3 py-2 focus:border-primary focus:ring-primary"
						/>
					</div>
					<div>
						...
						<input
							type="checkbox"
							id="is_active"
							bind:checked={currentItem.is_active}
						...
						<tr class="hover:bg-gray-50">
							...
							<td class="flex justify-center space-x-2 border px-4 py-3 text-center">
								<button
									...
									on:click={() => editItem(item)}>Editar</button
								>
								<button
									class="rounded bg-red-500 px-4 py-2 font-bold text-white hover:bg-red-600"
									on:click={() => deleteItem(item.id)}>Eliminar</button
								>
							</td>
						</tr>
					{/each}
