...
export async function GET({ url, locals }) {
  ...
  if (!bodega || !codigo_barras) {
    return errorResponse(
      400, 
      'BAD_REQUEST', 
      'Bodega and codigo_barras parameters are required'
    );
  }
  ...
  try {
    ...
    return errorResponse(500, 'INTERNAL_SERVER_ERROR', 'Error fetching inventory record', error.message);
  }
}
...
export async function PUT({ request, locals }) {
  ...
  try {
    ...
    return errorResponse(500, 'INTERNAL_SERVER_ERROR', 'Error updating inventory record', error.message);
  }
}