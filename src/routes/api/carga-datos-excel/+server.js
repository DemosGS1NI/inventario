...
function validateInventoryRecord(row, rowIndex) {
    ...
    if (row.inventario_sistema !== null && row.inventario_sistema !== undefined && row.inventario_sistema !== '') {
    ...
    if (row.single_item_ean13 && row.single_item_ean13.toString().length !== 13) {
        warnings.push(`Fila ${rowIndex}: Single Item EAN13 debería tener 13 dígitos para ser un EAN13 válido`);
    }

    if (row.master_carton_ean13 && row.master_carton_ean13.toString().length !== 13) {
        warnings.push(`Fila ${rowIndex}: Master Carton EAN13 debería tener 13 dígitos para ser un EAN13 válido`);
    }
...
export const POST = async ({ request, locals }) => {
    try {
        ...
        if (!['Admin', 'Supervisor'].includes(locals.user?.userRole)) {
            return errorResponse(403, 'FORBIDDEN', 'Solo administradores y supervisores pueden importar datos');
        }
        ...
        if (!data || data.length === 0) {
            return errorResponse(400, 'EMPTY_SHEET', 'La hoja de Excel está vacía o no contiene datos válidos');
        }
        ...
        if (existingRecordCount > 0) {
            ...
            return errorResponse(409, 'TABLE_NOT_EMPTY', 
                ...
                {
                    existingRecordCount,
                    instructions: 'Vaya a Administración → Limpieza de Tablas para limpiar los datos existentes antes de importar.'
                }
            );
        }
            ...
            if (row.id) {
                if (idsInFile.has(row.id)) {
                    ...
                    validationResults.invalidRecords.push({
                        rowNumber,
                        data: row,
                        errors: [`ID ${row.id} está duplicado en el archivo (también aparece en fila ${idsInFile.get(row.id)})`]
                    });
                    return;
                } else {
            ...
            if (row.codigo_barras) {
                if (codigosBarrasInFile.has(row.codigo_barras)) {
                    ...
                    validationResults.invalidRecords.push({
                        rowNumber,
                        data: row,
                        errors: [`Código de barras ${row.codigo_barras} está duplicado en el archivo (también aparece en fila ${codigosBarrasInFile.get(row.codigo_barras)})`]
                    });
                    return;
                } else {
        ...
        if (validationResults.invalidRecords.length > 0) {
            console.log(`Validation failed: ${validationResults.invalidRecords.length} invalid records found`);
            
            // Log details of validation errors for debugging
            console.log('Validation errors:', validationResults.invalidRecords.map(r => ({
                row: r.rowNumber,
                errors: r.errors,
        ...
        try {
            ...
            if (newCount !== validationResults.validRecords.length) {
                throw new Error(`Insert verification failed: expected ${validationResults.validRecords.length}, got ${newCount}`);
            }
            ...
            console.log(`Successfully imported ${newCount} records, replacing ${currentCount} previous records`);
