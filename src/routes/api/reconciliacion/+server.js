...
const reconciliationRecords = inventoryResult.rows.map(record => {
  ...
  const productMovements = movementsResult.rows.filter(
    movement => movement.codigo_barras === record.codigo_barras &&
...
console.log('Calculations:', {
  ...
  if (Math.abs(diferenciaReal) < 0.01) { // Account for floating point precision
    if (Math.abs(diferenciaAparente) > 0.01) {
      estadoReconciliacion = 'Explicado por Movimientos';
      explicacion = 'La diferencia aparente se explica completamente por los movimientos registrados.';
    } else {
  ...
  explicacion,
    ...
    const summary = {
      ...
      movementExplained: reconciliationRecords.filter(r => r.estadoReconciliacion === 'Explicado por Movimientos').length,
      trueDiscrepancies: reconciliationRecords.filter(r => r.estadoReconciliacion === 'Discrepancia Real').length,
      noDiscrepancy: reconciliationRecords.filter(r => r.estadoReconciliacion === 'Sin Diferencia').length
    };
  ...
  if (isExport) {
    try {
      ...
      return errorResponse(500, 'EXPORT_ERROR', 'Error generating Excel export', exportError.message);
    }
  }
  ...
  // Regular JSON response (not export)
    ...
    return errorResponse(500, 'INTERNAL_SERVER_ERROR', 'Error performing reconciliation analysis', error.message);
  }
}